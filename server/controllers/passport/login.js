...
exports.login = async (req, res) => {
    ...
    try {
        if (req.session.imageCaptcha === undefined) res.json({status: 0, msg: "服务器错误"});
            ...
            delete req.session.imageCaptcha;
            res.json({status: 0, msg: "验证码错误"});
        } else {
            let rows = await UserPass.selectOr([{phone: account}, {email: account}]);
            if (rows.length > 0) {
                //首先判断用户是否被ban（禁止登录）
                if (rows[0].banTime !== null) {
                    ...
                    if (rows[0].password !== encryption.encryptPassword(salt, password)) {
                        ...
                        if (failCount === 6) {  //超过最多连续输错密码次数
                            ...
                            await UserPass.update({userID}, {failCount: 6, banTime: date});
                            res.json({status: 0, msg: `您已连续输错密码6次，账号将被封禁至 ${date},可重置密码解锁账号`});
                        } else {  //连续输错密码
                            await UserPass.update({userID}, {failCount});
                            res.json({status: 0, msg: `密码错误(今日还剩${6 - failCount}次机会)`});
                        }
                    } else {    //密码正确
                        let date = moment().format('YYYY-MM-DD HH:mm:ss');
                        await UserPass.update({userID}, {
                            loginTime: date,
                            failCount: 0,
                            banTime: null
                        });
                        ...
                        req.session.level = rows[0].accessLevel;
