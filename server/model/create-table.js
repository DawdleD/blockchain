...
const CourseLive = sequelize.import('./course-live.js');

/**
 * 考试模块
 */
const CourseExam = sequelize.import('./course-exam.js');
const Examine = sequelize.import('./examine.js');
 ...
 * 一对一关系
 */
CourseExam.hasOne(CourseVideo, {foreignKey: 'examID'});
CourseVideo.belongsTo(CourseExam, {foreignKey: 'examID'});
/**
 * 每节课程对应一个期末测验
 * 一对一关系
 */
CourseExam.hasOne(CourseInformation, {foreignKey: 'examID'});
CourseInformation.belongsTo(CourseExam, {foreignKey: 'examID'});
/**
 * 每门考试有多个学生参加，每个学生可以参加多门考试
 * 多对多
 */
CourseExam.belongsToMany(UserInformation, {through: Examine, foreignKey: 'examID'});
UserInformation.belongsToMany(CourseExam, {through: Examine, foreignKey: 'userID'});
/**
 * 每节课程（直播）对应一个直播视频
 * 一对一
 */
CourseInformation.hasOne(CourseLive, {foreignKey: 'courseID'});
CourseLive.belongsTo(CourseInformation, {foreignKey: 'courseID'});
/**
 * Project 部分
 */
...
/*课程与支付为一对一关系*/
CourseInformation.hasOne(ProjectPaymentRecord, {foreignKey: 'courseID'});
ProjectPaymentRecord.belongsTo(CourseInformation, {foreignKey: 'courseID'});
...
UserInformation.belongsToMany(ProjectInformation, {
    through: 'ProjectMember',
    foreignKey: 'memberID',
    as: 'AttendProjects'
})
ProjectInformation.belongsToMany(UserInformation, {
    through: 'ProjectMember',
...
module.exports = {
    ...
    CourseInformation, CourseClass, CourseComment, CourseChapter, CourseVideo,
    CourseType, CourseSystem, CourseDetail, CourseFile, CourseLive,
    CourseExam, Examine,
